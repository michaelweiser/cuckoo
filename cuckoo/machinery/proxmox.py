...
class Proxmox(Machinery):
    ...
    def wait_for_task(self, taskid, vm, node):
        ...
        while elapsed < self.timeout:
            ...
            if task["status"] != "stopped":
                log.debug("%s: Waiting for operation %s (%s) to finish: %s",
    ...
    def rollback(self, vm, node):
        ...
        task = self.wait_for_task(taskid, vm, node)
        if not task:
